    ...
    (dolist (field fields
      ...
      (destructuring-bind (&key from len &allow-other-keys) field
        (destructuring-bind (row column) from
          (push (make-declared-field field
				     (subseq (aref static-row-definitions row)
                                             column (+ len column)))
          ...
          (if (< (+ column len 1) +screen-columns+)
              (push `(make-field :row ,row
				 :col ,(+ column len 1))
		    (aref row-to-fields 0))
	      
              (when (< (+ row 1) +screen-rows+)
                (push `(make-field :row ,(1+ row)
				   :col 0)
		      (aref row-to-fields 0)))))))))
...
(defun get-static-fields (row string)
  (let (fields)
    (ppcre:do-matches (start end "\\S+(?: \\S+)*" string (nreverse fields))
      (push `(make-field :row ,row
			 :col ,start
			 :content ,(subseq string start end))
	    fields))))


(defun parse-screen (screen fields)
  (let* ((static-row-definitions
	   (coerce (rest (split-sequence:split-sequence #\Newline screen))
		   'vector))
         (row-to-field-map
	   (make-row-to-field-map fields static-row-definitions)))
...
(defmacro defscreen (name screen &key fields)
  `(defparameter ,name
     (make-screen ,(string name) ,@(parse-screen screen fields))))


;;;; defscree.lisp ands here.
